...
#this function uses the lasso algorithm for automatic feature selection.
...
def park_lasso():
    ...
    try:
        ...
        # use regularization strength of 0.01
        alpha_value = 0.01
        ...
        if not selected_features:
         ...
         # train a model on only selected features
...
#this function is to train the model using the features selected by the administrator
...
#this function is to display the parkinsons disease prediction form
@parkinsons_routes.route("/parkinsonsdisease")
def parkinsons_disease():
    return render_template("parkinsonsdisease.html")

#this function is to upload the parkinsons dataset
...
#this function is to get all data about the patient.
...
#this function is predict if the patient has parkinsons disease or not.
...
#this function is get all the feature contributions present in the patient.
...
# this function is get the all the predictions made by the system.  
def get_all_patient_data():
    try:
        # Query all patient records from the database
...
#this function is to get all the symptoms present in the patient.
...
#this function is to view the system predicted results.
...
#this function is to view the symptoms present in the patient.
...
#this function is to view the feature contributions towards the prediction result.
...
def view_parkinsons_contributions():
    ...
    return render_template("parkinsons_contributions.html", contributions=all_contributions)
...
def update_status():
    ...
    if send_to_patient:
        ...
        diagnosis_text = "Positive for Parkinsons, please book an appointment to see Dr Fredrick immediately using our onlines booking portal" if final_diagnosis == '1' else "Negative for Parkinsons" if final_diagnosis == '0' else "Further testing recommended. Please visit the clinic for additional evaluation."
...
@parkinsons_routes.route("/parkinsonsexplanation", methods=["POST"])
def parkinsons_explanation():
    ...
    print(contributions)
    explanation = f"Hello,\n\n"
    explanation += "Here is your Parkinsons risk analysis:\n\n"

    for c in contributions:
        if c['effect'] == "increased":
            explanation += f"- Your {c['feature']} is **high**, which may increase your risk for parkinsons.\n"
        elif c['effect'] == "lowered":
            explanation += f"- Your {c['feature']} is **low**, which may lower your risk for parkinsons.\n"
        else:
            explanation += f"- Your {c['feature']} has **no significant effect** your risk for parkinsons.\n"
...
@parkinsons_routes.route("/view_explanation_parkinsons")
def view_explanation_parkinsons():
