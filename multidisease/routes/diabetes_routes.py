...
def train_lasso():
    ...
    try:
        ...
        # STEP 2: Train a NEW model on only selected features 👈 CHANGED
        X_train_selected = X_train[selected_features]  # 👈 CHANGED
        X_test_selected = X_test[selected_features]    # 👈 CHANGED

        diabetes_model = LogisticRegression(penalty='l1', solver='liblinear', C=alpha_value, max_iter=5000)  # 👈 CHANGED
        diabetes_model.fit(X_train_selected, Y_train)  # 👈 CHANGED

        # Save selected features (including Outcome)
        ...
        joblib.dump(diabetes_model, "uploads/diabetes_model.pkl")  # 👈 still fine
        ...
        if diabetes_data:
            ...
            for diabetes in diabetes_data:
                diabetes_record = {
                    ...
                    'BloodPressure': diabetes.Blood_pressure,  # Ensure consistent naming
...
def update_result():
    ...
    if "savedata" in request.form and selected_rows:
        ...
        try:
            # Open the file in append mode with newline="" to avoid formatting issues
    ...
    if send_to_patient:
        ...
        diagnosis_text = "Positive for diabetes" if outcome == '1' else "Negative for diabetes" if outcome == '0' else "To be determined"
